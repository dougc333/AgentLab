...
miniwob: stop-miniwob
	...
	@uv run python -m http.server 8000 --directory miniwob-plusplus/miniwob/html & echo $$! > .miniwob-server.pid
	@sleep 3
	@echo "MiniWob server started on http://localhost:8000"

check-miniwob:
	@curl -I "http://localhost:8000/miniwob/" || (echo "MiniWob not reachable" && exit 1)
...
run-tests:
	@MINIWOB_URL="http://localhost:8000/miniwob/" uv run pytest -n 5 --durations=10 -m 'not pricy' tests/
